...
import { updateManualContract, searchCustomers } from '@/lib/actions';
...
export default function EditContractPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [fineRate, setFineRate] = useState(200);
    ...
    useEffect(() => {
        async function load() {
            try {
                    ...
                    .select(`
                        *,
                        customer_locations (
                            ...
                            longitude,
                            distance_km,
                            customers ( name, phone, gra_number, license_number )
                        )
                    `)
                ...
                if (error || !c) {
                    alert('Contract not found or error loading');
                ...
                // Populate Form
                setForm({
                    licenseNumber: c.customer_locations?.customers?.license_number || '',
                    graNumber: c.customer_locations?.customers?.gra_number || '',
                    locationName: c.customer_locations?.display_name || '',
                    phone: c.customer_locations?.customers?.phone || '',
                    customerName: c.customer_locations?.customers?.name || '',
                    latitude: c.customer_locations?.latitude?.toString() || '',
                    longitude: c.customer_locations?.longitude?.toString() || '',
                    ...
                    amcValue: c.amc_value?.toString() || '',
                    fineAmount: c.fine_amount?.toString() || '0.00',
                    amount: c.amount_total?.toString() || '',
                    paidAmount: c.paid_amount?.toString() || '0.00',
                    balanceAmount: c.balance_amount?.toString() || ''
                });

                // Infer Initial Duration? Can't really, just default to 1 Year or user manual entry
                // If existing dates are set, better NOT to auto-overwrite them immediately unless user interacts
    ...
    function handleChange(e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) {
        ...
        if (name !== 'customerName') {
            setForm((prev) => {
                ...
                // If Start Date changes, update Renewal Date based on current Duration
                if (name === 'amcDate') {
                     ...
                     if (!isNaN(start.getTime())) {
                        ...
                        updated.renewalDate = end.toISOString().split('T')[0];
                     }
                }
    ...
    // Dedicated Duration Change Handler
    function handleDurationChange(e: React.ChangeEvent<HTMLSelectElement>) {
        ...
        if (form.amcDate) {
             ...
             if (!isNaN(start.getTime())) {
                const end = new Date(start);
                end.setMonth(start.getMonth() + newDur);
                end.setDate(end.getDate() - 1);
                setForm(prev => ({ ...prev, renewalDate: end.toISOString().split('T')[0] }));
             }
        }
    }

    // Manual Recalculate Fine Button
    function recalcFine() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            await updateManualContract(Number(resolvedParams.id), formData);
            router.push(`/manager/contracts/${resolvedParams.id}`);
        } catch (err: any) {
    ...
    return (
        <div className="max-w-xl mx-auto py-10">
            <h1 className="text-2xl font-bold text-gray-900 mb-6 font-mono text-center">Edit Contract #{resolvedParams.id}</h1>
            <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-lg shadow-md text-sm border border-gray-200" onClick={() => setSuggestions([])}>
                ...
                <div className="flex gap-4">
                     <button type="button" onClick={() => router.back()} className="w-1/3 px-4 py-3 bg-gray-200 text-gray-700 rounded font-bold hover:bg-gray-300 transition-colors">
                        Cancel
                    </button>
                    <button type="submit" disabled={saving} className="w-2/3 px-4 py-3 bg-blue-600 text-white rounded font-bold hover:bg-blue-700 shadow-md transition-colors disabled:opacity-50">
